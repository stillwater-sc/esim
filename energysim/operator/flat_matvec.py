...
def flat_matvec_gpu(rows, cols, attributes: 'GraphicsProcessingUnitEnergy', config: 'GraphicsProcessingUnitConfiguration') \
    ...
    gpu_metrics.record('l1_write', total_cache_lines, attributes.l1_write)

    gpu_metrics.record('l2_read', total_cache_lines_in, attributes.l2_read)
    gpu_metrics.record('l2_write', total_cache_lines, attributes.l2_write)

    gpu_metrics.record('l3_read', total_cache_lines_in, attributes.l3_read)
    gpu_metrics.record('l3_write', total_cache_lines, attributes.l3_write)

    # for the DRAM, we assume just the energy for reading the operands for compute,
    # and do not include the energy required for memory management
    # to get the data structures into memory
    gpu_metrics.record('dram_read', total_cache_lines_in, attributes.dram_read)
    gpu_metrics.record('dram_write', total_cache_lines_out, attributes.dram_write)
    ...
    gpu_metrics.consolidate('l2', ['l2_read', 'l2_write'])
    gpu_metrics.consolidate('l3', ['l3_read', 'l3_write'])
    gpu_metrics.consolidate('cache_read', ['l1_read', 'l2_read', 'l3_read'])
    gpu_metrics.consolidate('cache_write', ['l1_write', 'l2_write', 'l3_write'])
    gpu_metrics.consolidate('cache', ['cache_read', 'cache_write'])
    gpu_metrics.consolidate('memory', ['dram_read', 'dram_write'])
